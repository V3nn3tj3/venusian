...
if ! public_key_string=$(sudo ssh-keygen -y -f /data/home/vnctunnel/.ssh/id_rsa); then
  exit 0
fi
...
while read -r service; do
  ...
  addr=$(dbus-send --print-reply=literal --session --dest="$service" \
...
done <<< "$(dbus-send --print-reply --session --dest=org.freedesktop.DBus \
...
{
    if ! settingsurl_val=$(dbus-send --print-reply=literal --session \
...
vrm_portal=$(dbus-send --print-reply=literal --session --dest=com.victronenergy.settings \
               /Settings/Network/VrmPortal com.victronenergy.BusItem.GetValue | awk '{print $3}')

if [ "$vrm_portal" = "2" ]; then
  for port in 80 1880 3000 5900; do
    permitopen+=("localhost:${port}" "127.0.0.1:${port}")
  done
